    ...
    def __init__(self, win, credentials_controller: CredentialsController, event_controller: EventController):
        ...
        def validate_login():
            ...
            if user.role == RoleName.ADMIN.name or user.role == RoleName.HOST.name:
                ...
                event_root.mainloop()
            elif user.role == RoleName.ANONYMOUS_USER.name:
                event_root = Tk()
                event_controller.reload_events()
        ...
        def signup():
            root = Tk()
            login_view = MainRegisterView(root, credentials_controller, event_controller)
        ...
        sign_up_btn = Button(win, text="Switch To Sign up", command=partial(signup))
